              ...
              <div className="px-4 py-6 flex flex-col space-y-4">
                {links.map((link) => (
                  <a
                    ...
                    className="text-lg font-medium text-saphir py-2 border-b border-gray-100 last:border-0 hover:text-cyan transition-colors"
                  >
                    {link.label}
                  </a>
                ))}
                <a
                  href="#waitlist"
                  onClick={() => setMobileOpen(false)}
                  className="mt-2 w-full text-center inline-flex items-center justify-center px-6 py-3 rounded-xl text-base font-semibold text-white bg-saphir hover:bg-saphir-light transition-all duration-300"
                >
                  Accès bêta gratuit →
                </a>
              </div>
